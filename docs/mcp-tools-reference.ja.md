...
MoMorph 内のデザインデータ、specs、test cases、media、ノードツリーを操作するために MoMorph MCP server が公開する **全 31 ツール** の完全なリファレンスです。
...
- Screen 情報の返却内容: `screen_id, name, status, revision (commit_hash), tags, created_at, updated_at`
...
| 4   | [`list_frames`](#4-list_frames)                            | Specs        | すべての Screen を一覧表示（status による絞り込みは任意）                                     |
| 5   | [`list_frame_sets`](#5-list_frame_sets)                    | Specs        | すべての ScreenSet を一覧表示                                                                |
| 6   | [`get_frame_set`](#6-get_frame_set)                        | Specs        | ScreenSet とその子 Screen を取得                                                             |
| 7   | [`upload_specs`](#7-upload_specs)                          | Specs        | Screen の specs をアップロード / 更新                                                        |
| 8   | [`download_specs`](#8-download_specs)                      | Specs        | Screen の specs を CSV / JSON にエクスポート                                                 |
| 9   | [`list_design_items`](#9-list_design_items)                | Specs        | Screen のすべての specs を一覧表示                                                           |
| 10  | [`list_frame_spec_diffs`](#10-list_frame_spec_diffs)       | Specs        | 最新リビジョンに対する spec の変更を比較                                                      |
| 11  | [`get_related_design_items`](#11-get_related_design_items) | Specs        | Screen に関連する specs を取得（存在する場合）                                                |
| 12  | [`upload_test_cases`](#12-upload_test_cases)               | Test Cases   | Screen の test cases をアップロード / 更新                                                   |
| 13  | [`download_test_cases`](#13-download_test_cases)           | Test Cases   | test cases を CSV / JSON にエクスポート                                                      |
| 14  | [`get_frame_test_cases`](#14-get_frame_test_cases)         | Test Cases   | Screen の test cases を取得                                                                  |
| 15  | [`get_frame_image`](#15-get_frame_image)                   | Media        | Screen のプレビュー画像を取得（spec 番号のオーバーレイ付きも可）                              |
| 16  | [`list_media_items`](#16-list_media_items)                 | Media        | Screen 内の media nodes を一覧表示                                                           |
| 17  | [`list_media_nodes`](#17-list_media_nodes)                 | Media        | role ヒント付きですべての media nodes を一覧表示（background/icon/overlay/...）              |
| 18  | [`get_media_files`](#18-get_media_files)                   | Media        | Screen 内のすべての media assets（SVG/PNG/JPG）のダウンロード URL を取得                      |
| 19  | [`get_media_file`](#19-get_media_file)                     | Media        | ノードの media asset（SVG/PNG/JPG）をストレージから直接ダウンロード                           |
| 20  | [`get_figma_image`](#20-get_figma_image)                   | Media        | 任意の Figma ノードを PNG/JPG にエクスポート（Figma の rate-limit に達する可能性あり）        |
| 21  | [`get_design_item_image`](#21-get_design_item_image)       | Media        | 特定の spec item（design item）のクロップ画像を取得                                           |
| 22  | [`get_frame_node_tree`](#22-get_frame_node_tree)           | Figma Design | Screen の完全な階層ノードツリーを取得                                                         |
| 23  | [`get_overview`](#23-get_overview)                         | Figma Design | 軽量な概要ツリー（名前 + タイプのみ）を取得 — 最初に使用して全体像を把握                       |
| 24  | [`query_section`](#24-query_section)                       | Figma Design | ノード ID またはあいまいな名前でサブツリーをクエリ                                            |
| 25  | [`query_component`](#25-query_component)                   | Figma Design | 名前でコンポーネントを検索（あいまい一致、関連度でランク付け）                                |
| 26  | [`query_by_type`](#26-query_by_type)                       | Figma Design | Figma タイプですべてのノードをフィルタ（TEXT、INSTANCE、FRAME、...）                          |
| 27  | [`get_node`](#27-get_node)                                 | Figma Design | 正確なノード ID で単一ノードを取得                                                            |
| 28  | [`get_node_context`](#28-get_node_context)                 | Figma Design | parent・siblings・レイヤー role 分析付きでノードを取得（画像レンダリング前に必須）             |
| 29  | [`list_file_variables`](#29-list_file_variables)           | Figma Design | ファイルの CSS / design variables を一覧表示                                                 |
| 30  | [`list_frame_styles`](#30-list_frame_styles)               | Figma Design | Screen の完全な階層 CSS スタイルツリーを取得                                                  |
| 31  | [`list_file_localizations`](#31-list_file_localizations)   | Figma Design | ソーステキスト文字列に対する多言語翻訳を取得                                                  |
...
| `designStatus`   | enum   | ✗    | `none`     | design フェーズのステータス: `none` \| `in_progress` \| `done` |
| `specStatus`     | enum   | ✗    | `none`     | spec フェーズのステータス: `none` \| `in_progress` \| `done`   |
| `devStatus`      | enum   | ✗    | `none`     | dev フェーズのステータス: `none` \| `developing` \| `done`     |
| `reviewStatus`   | enum   | ✗    | `none`     | review フェーズのステータス: `none` \| `in_progress` \| `done` |
...
`screenId` で Screen の詳細情報を取得します。基本的なメタデータ（name、status、revision（commit hash）、tags、タイムスタンプ）を返します。
...
**Response (JSON):** `screen_id, name, status, revision (commit_hash), created_at, updated_at` を含む frame オブジェクト。

---

### 4. `list_frames`

ファイル内のすべての Screen を一覧表示します。デフォルトではすべての Screen を返しますが、status（`design_status`, `spec_status`, `dev_status`, `draft`, `completed`）で任意に絞り込めます。各 Screen は一意な識別子として `screen_id` を含みます。
...
| --------- | -------- | ---- | -------------------------------------------------------------------------------------------------- |
| `fileKey` | string   | ✓    | Figma file key                                                                                     |
| `status`  | string[] | ✗    | `design_status`, `spec_status`, `dev_status`, `draft`, `completed` のサブセット。すべての frame を取得するには省略します。 |
