...
Complete reference for all **31 tools** that the MoMorph MCP server exposes for interacting with design data, specs, test cases, media, and node tree in MoMorph.
...
- Screen info returns: `screen_id, name, status, revision (commit_hash), tags, created_at, updated_at`
...
| 4   | [`list_frames`](#4-list_frames)                            | Specs        | List all Screens (filter by status optional)                                                |
| 5   | [`list_frame_sets`](#5-list_frame_sets)                    | Specs        | List all ScreenSets                                                                         |
| 6   | [`get_frame_set`](#6-get_frame_set)                        | Specs        | Get a ScreenSet with its child Screens                                                      |
| 7   | [`upload_specs`](#7-upload_specs)                          | Specs        | Upload / update specs for a Screen                                                          |
| 8   | [`download_specs`](#8-download_specs)                      | Specs        | Export Screen specs to CSV / JSON                                                           |
| 9   | [`list_design_items`](#9-list_design_items)                | Specs        | List all specs of a Screen                                                                  |
| 10  | [`list_frame_spec_diffs`](#10-list_frame_spec_diffs)       | Specs        | Compare spec changes vs latest revision                                                     |
| 11  | [`get_related_design_items`](#11-get_related_design_items) | Specs        | Get related specs for a Screen (if any)                                                     |
| 12  | [`upload_test_cases`](#12-upload_test_cases)               | Test Cases   | Upload / update test cases for a Screen                                                     |
| 13  | [`download_test_cases`](#13-download_test_cases)           | Test Cases   | Export test cases to CSV / JSON                                                             |
| 14  | [`get_frame_test_cases`](#14-get_frame_test_cases)         | Test Cases   | Get test cases of a Screen                                                                  |
| 15  | [`get_frame_image`](#15-get_frame_image)                   | Media        | Get Screen preview image, optionally with spec number overlay                               |
| 16  | [`list_media_items`](#16-list_media_items)                 | Media        | List media nodes in a Screen                                                                |
| 17  | [`list_media_nodes`](#17-list_media_nodes)                 | Media        | List all media nodes with role hints (background/icon/overlay/...)                          |
| 18  | [`get_media_files`](#18-get_media_files)                   | Media        | Get download URLs for all media assets (SVG/PNG/JPG) in a Screen                            |
| 19  | [`get_media_file`](#19-get_media_file)                     | Media        | Download a media asset (SVG/PNG/JPG) of a node directly from storage                        |
| 20  | [`get_figma_image`](#20-get_figma_image)                   | Media        | Export any Figma node to PNG/JPG (may hit Figma rate-limit)                                 |
| 21  | [`get_design_item_image`](#21-get_design_item_image)       | Media        | Get a cropped image of a specific spec item (design item)                                   |
| 22  | [`get_frame_node_tree`](#22-get_frame_node_tree)           | Figma Design | Get the full hierarchical node tree of a Screen                                             |
| 23  | [`get_overview`](#23-get_overview)                         | Figma Design | Get a lightweight overview tree (names + types only) — use first to orient                  |
| 24  | [`query_section`](#24-query_section)                       | Figma Design | Query a subtree by node ID or fuzzy name                                                    |
| 25  | [`query_component`](#25-query_component)                   | Figma Design | Find a component by name (fuzzy match, ranked by relevance)                                 |
| 26  | [`query_by_type`](#26-query_by_type)                       | Figma Design | Filter all nodes by Figma type (TEXT, INSTANCE, FRAME, ...)                                 |
| 27  | [`get_node`](#27-get_node)                                 | Figma Design | Get a single node by exact node ID                                                          |
| 28  | [`get_node_context`](#28-get_node_context)                 | Figma Design | Get a node with parent, siblings, and layer role analysis (required before rendering image) |
| 29  | [`list_file_variables`](#29-list_file_variables)           | Figma Design | List CSS / design variables of the file                                                     |
| 30  | [`list_frame_styles`](#30-list_frame_styles)               | Figma Design | Get the full hierarchical CSS style tree of a Screen                                        |
| 31  | [`list_file_localizations`](#31-list_file_localizations)   | Figma Design | Get multilingual translations for source text strings                                       |
...
| `designStatus`   | enum   | ✗        | `none`  | Design phase status: `none` \| `in_progress` \| `done` |
| `specStatus`     | enum   | ✗        | `none`  | Spec phase status: `none` \| `in_progress` \| `done`   |
| `devStatus`      | enum   | ✗        | `none`  | Dev phase status: `none` \| `developing` \| `done`     |
| `reviewStatus`   | enum   | ✗        | `none`  | Review phase status: `none` \| `in_progress` \| `done` |
...
Get full details for a Screen by `screenId`. Returns basic metadata: name, status, revision (commit hash), tags, and timestamps.
...
**Response (JSON):** frame object with `screen_id, name, status, revision (commit_hash), created_at, updated_at`.

---

### 4. `list_frames`

List all Screens in a file. By default returns ALL Screens; optionally filter by status (`design_status`, `spec_status`, `dev_status`, `draft`, `completed`). Each Screen includes `screen_id` as the unique identifier.
...
| --------- | -------- | -------- | -------------------------------------------------------------------------------------------------- |
| `fileKey` | string   | ✓        | Figma file key                                                                                     |
| `status`  | string[] | ✗        | Subset of `design_status`, `spec_status`, `dev_status`, `draft`, `completed`. Omit for ALL frames. |
