...
Tài liệu tham khảo đầy đủ cho **32 tools** mà MoMorph MCP server cung cấp để tương tác với dữ liệu thiết kế, specs, test cases, media, và node tree trong MoMorph.
...
- Screen info trả về: `screen_id, name, design_status, spec_status, dev_status, review_status, revision (commit_hash), tags, created_at, updated_at`
...
| 4   | [`update_frame`](#4-update_frame)                          | Specs        | Cập nhật metadata của Screen (name / screen_overview / 4 sub-status)                     |
| 5   | [`list_frames`](#5-list_frames)                            | Specs        | Liệt kê tất cả Screen (có thể lọc theo 4 sub-status)                                     |
| 6   | [`list_frame_sets`](#6-list_frame_sets)                    | Specs        | Liệt kê tất cả ScreenSet                                                                 |
| 7   | [`get_frame_set`](#7-get_frame_set)                        | Specs        | Lấy chi tiết một ScreenSet kèm các Screen con                                            |
| 8   | [`upload_specs`](#8-upload_specs)                          | Specs        | Upload / cập nhật specs cho Screen                                                       |
| 9   | [`download_specs`](#9-download_specs)                      | Specs        | Xuất specs của Screen ra CSV / JSON                                                      |
| 10  | [`list_design_items`](#10-list_design_items)               | Specs        | Liệt kê tất cả specs của Screen                                                          |
| 11  | [`list_frame_spec_diffs`](#11-list_frame_spec_diffs)       | Specs        | So sánh thay đổi của specs so với revision gần nhất                                      |
| 12  | [`get_related_design_items`](#12-get_related_design_items) | Specs        | Lấy các specs liên quan cho một Screen (nếu có)                                          |
| 13  | [`upload_test_cases`](#13-upload_test_cases)               | Test Cases   | Upload / cập nhật test cases cho Screen                                                  |
| 14  | [`download_test_cases`](#14-download_test_cases)           | Test Cases   | Xuất test cases ra CSV / JSON                                                            |
| 15  | [`get_frame_test_cases`](#15-get_frame_test_cases)         | Test Cases   | Lấy test cases của Screen                                                                |
| 16  | [`get_frame_image`](#16-get_frame_image)                   | Media        | Lấy ảnh preview của Screen hoặc ảnh có đánh số specs                                     |
| 17  | [`list_media_items`](#17-list_media_items)                 | Media        | Liệt kê các media nodes có trong Screen                                                  |
| 18  | [`list_media_nodes`](#18-list_media_nodes)                 | Media        | Liệt kê tất cả media nodes kèm gợi ý role (background/icon/overlay...)                   |
| 19  | [`get_media_files`](#19-get_media_files)                   | Media        | Lấy URL download cho toàn bộ media assets (SVG/PNG/JPG) trong Screen                     |
| 20  | [`get_media_file`](#20-get_media_file)                     | Media        | Tải trực tiếp file media asset (SVG/PNG/JPG) của một node từ storage                     |
| 21  | [`get_figma_image`](#21-get_figma_image)                   | Media        | Export một Figma node bất kỳ thành ảnh PNG/JPG (có thể bị dính Figma rate-limit)         |
| 22  | [`get_design_item_image`](#22-get_design_item_image)       | Media        | Lấy ảnh đã crop của một spec item (design item) cụ thể                                   |
| 23  | [`get_frame_node_tree`](#23-get_frame_node_tree)           | Figma Design | Lấy cây node phân cấp đầy đủ của Screen                                                  |
| 24  | [`get_overview`](#24-get_overview)                         | Figma Design | Lấy cây tổng quan gọn nhẹ (chỉ tên + type, không style) — dùng đầu tiên để định hình     |
| 25  | [`query_section`](#25-query_section)                       | Figma Design | Truy vấn subtree theo node ID hoặc fuzzy name                                            |
| 26  | [`query_component`](#26-query_component)                   | Figma Design | Tìm component theo tên (fuzzy match, xếp hạng theo độ liên quan)                         |
| 27  | [`query_by_type`](#27-query_by_type)                       | Figma Design | Lọc tất cả nodes theo type Figma (TEXT, INSTANCE, FRAME, ...)                            |
| 28  | [`get_node`](#28-get_node)                                 | Figma Design | Lấy chi tiết một node theo node ID chính xác                                             |
| 29  | [`get_node_context`](#29-get_node_context)                 | Figma Design | Lấy một node kèm parent, siblings, và phân tích role layers (cần trước khi render image) |
| 30  | [`list_file_variables`](#30-list_file_variables)           | Figma Design | Lấy danh sách CSS / design variables của file                                            |
| 31  | [`list_frame_styles`](#31-list_frame_styles)               | Figma Design | Lấy cây style CSS phân cấp của Screen                                                    |
| 32  | [`list_file_localizations`](#32-list_file_localizations)   | Figma Design | Lấy bản dịch đa ngôn ngữ cho các chuỗi text nguồn                                        |
...
| `name`           | string | ✓        | —        | Tên Screen (không được để trống)                                                                                                                                                                                               |
| `screenOverview` | string | ✗        | —        | Mô tả / tổng quan của screen                                                                                                                                                                                                   |
| `figmaNodeId`    | string | ✗        | —        | Figma node ID screen này link tới. Chấp nhận 3 dạng: canonical `"12318:23788"`, hyphen `"12318-23788"`, hoặc full Figma URL có `node-id=` — đều được normalize về canonical. Bỏ qua hoặc truyền `""` nếu chưa liên kết.         |
| `designStatus`   | enum   | ✗        | —        | Design phase status: `none` \| `in_progress` \| `done`                                                                                                                                                                         |
| `specStatus`     | enum   | ✗        | —        | Spec phase status: `none` \| `in_progress` \| `done`                                                                                                                                                                           |
| `devStatus`      | enum   | ✗        | —        | Dev phase status: `none` \| `developing` \| `done` ⚠ (khác 3 cái còn lại)                                                                                                                                                      |
| `reviewStatus`   | enum   | ✗        | —        | Review phase status: `none` \| `in_progress` \| `done`                                                                                                                                                                         |

> **Breaking change:** Trường `status` cũ (deprecated frame_status) đã được loại bỏ khỏi input lẫn response. Dùng 4 sub-status.
...
Lấy thông tin chi tiết của một Screen theo `screenId`. Trả về metadata: tên, 4 sub-status (design/spec/dev/review), screen_overview, figma_node_id, revision (commit hash), thời gian tạo/cập nhật.
...
> **Breaking change:** Field `status` (deprecated `frame_status`) đã bị bỏ khỏi response — dùng 4 sub-status bên trên.

---

### 4. `update_frame`

Cập nhật một số trường metadata của Screen. Patch semantics: chỉ field được pass mới được update; field bỏ qua giữ nguyên. Pass `screenOverview: ""` (chuỗi rỗng) để clear (set null).
...
| ---------------- | ------ | -------- | ---------------------------------------------------------------------------------------------------------------------- |
| `screenId`       | string | ✓        | Screen ID                                                                                                              |
| `name`           | string | ✗        | Tên Screen mới (không được để trống)                                                                                   |
| `screenOverview` | string | ✗        | Overview/description mới. Pass `""` để clear (set null).                                                               |
| `figmaNodeId`    | string | ✗        | Figma node ID screen này link tới. Chấp nhận canonical/hyphen/URL (xem `create_frame`). Pass `""` để unlink (set null). |
...
- Phải pass ít nhất 1 field ngoài `screenId`, không thì trả error `"No fields to update"`.
- Nếu `screenId` không khớp Screen nào → trả error `"Frame not found for screenId: ..."`.
- Tool **không** expose trường `status` cũ — chỉ 4 sub-status.
...
- Chỉ đổi tên: `{ screenId, name: "New name" }`
- Clear overview: `{ screenId, screenOverview: "" }`
- Đánh dấu dev xong: `{ screenId, devStatus: "done" }`
- Mix: `{ screenId, name: "X", screenOverview: "Y", specStatus: "done" }`

---

### 5. `list_frames`

Liệt kê tất cả Screen trong một file. Mặc định trả về TẤT CẢ (loại trừ Screen đã archive); có thể lọc theo từng sub-status (design/spec/dev/review). Nhiều filter AND-combined.
...
| `devStatus`       | enum    | ✗        | —        | `none` \| `developing` \| `done` ⚠                                 |
| `reviewStatus`    | enum    | ✗        | —        | `none` \| `in_progress` \| `done`                                  |
| `includeOverview` | boolean | ✗        | `false`  | Khi `true`, response mỗi Screen kèm `screen_overview` (có thể lớn) |

> **Breaking change:** Trường `status` cũ (deprecated frame_status) đã bị loại bỏ khỏi input filter lẫn response. Phiên bản trước docs còn liệt kê `draft`/`completed` — đó là bug. Dùng 4 sub-status filter mới.
...
```ts
{
  frames: Array<{
    ...
    tags: string[];
    screen_overview?: string | null;          // chỉ khi includeOverview=true
    created_at: string;
    updated_at: string;
  }>;
}
...
| `node_link_id`     | string                    | △¹       | Figma node ID (UI Part). CSV: `itemId`. Bắt buộc khi liên kết với design Figma                                                                                                                          |
...
> **¹ Điều kiện identity (`△`):** Mỗi item cần **ít nhất một** trong `no`, `name`, `nameTrans` hoặc `node_link_id` (UI Part) có giá trị thì mới được lưu — **không** bắt buộc điền tất cả. Các field khác là optional.
