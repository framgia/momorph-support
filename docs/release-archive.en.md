...
- **New Screen Spec sort & reorder** — 3-state sort cycle (↕ → ASC → DESC → ↕) with preview-only behavior; sort and drag-drop now share the table's Save/Cancel. Empty `No` always sinks to the bottom; dotted `No` sorts lexically.
- **Maintenance mode** — A dedicated maintenance screen is now shown during release windows.

**🐛 Fixes**

- Items with missing UI Part now stay visible on Item List after reloading the preview.
- Cancelling a queued AI spec generation now correctly returns the item to `draft` instead of flipping back to `generating`.
...
- Item spec form now supports ID input and direct Figma layer linking. Layer names auto-prefix with "mms\_".
- Figma layer prefix unified to "mms\_" across the app. Old formats convert automatically.
