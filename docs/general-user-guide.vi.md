...
> **Dành cho:** Designer, BrSE, Dev, QA, PM dùng workflow AI-Driven Development của MoMorph
...
<details>
<summary><strong>4. <a href="#4-momorph-syncer-google-add-on---chỉ-dành-cho-nhân-viên-sun">MoMorph Syncer (Google Add-on)</a></strong></summary>
...
| 6   | **MCP Server**               | Cung cấp 31 AI tools cho agent đọc/ghi dữ liệu MoMorph & Figma                              | Tự động qua AI agent |
...
- Domain `sun-asterisk.com` đã trong whitelist Pro mặc định → mọi tài khoản Sun\* nhận Pro tự động
- Email khác: liên hệ Slack `#con_momorph-support_all` để được add whitelist
...
| Syncer (Google Add-on)   | Google  | Tài khoản **Sun\* Asterisk Google Workspace** (`@sun-asterisk.com`)                                                                                                |
...
Plugin ID prod: `1406117276934709483`.
...
> Tài khoản Figma phải nằm trong whitelist Pro (xem §1.3). Nếu chưa, browser sẽ báo lỗi sau khi authorize.
...
> **Lỡ chọn Essential dù có tài khoản Pro?** Plugin sẽ ghi nhớ lựa chọn này, lần mở sau người dùng sẽ vào thẳng Essential mà không hiện lại màn Welcome. Để quay về màn Welcome, cần **xoá thông tin gói được lưu tại clientStorage của Figma**. Sau khi xoá, đóng plugin và mở lại — màn Welcome sẽ xuất hiện, click **"Đăng nhập với tài khoản Pro"**.
...
3. **Click icon Refresh** (🔄) trên thanh **Preview toolbar** (tooltip _"Làm mới ảnh"_) khi:
...
3. Authorize Figma popup
4. Lần đầu: nhập `fileKey` để link

Link **GitHub repo** (cho VSCode Extension/CLI): file → **Settings → GitHub** → Connect → chọn repo.

### 3.3 Core Features

Điều hướng qua sidebar/breadcrumb. Routes liệt kê dưới đây để verify URL — mở trên `momorph.ai/<route>` sau khi login.
...
| Screen list      | `/files/{fileKey}/screens`                 | Screens trong file (filter theo status, tag, page)             |
| Screen detail    | `/files/{fileKey}/screens/{screenId}`      | Preview ảnh + design items                                     |
| Item spec editor | `…/screens/{screenId}/items/{itemId}/spec` | Edit validation/navigation/DB mapping; có **AI Generate Spec** |
| Spec revisions   | `…/items/{itemId}/revs` · `…/revs/{revId}` | Lịch sử version + rollback                                     |
| View all specs   | `…/screens/{screenId}/items/all-specs`     | Bảng read-only xem tất cả specs trong screen                   |
| Screen tags      | `…/screens/{screenId}/tags`                | Tag screen (functional, bug…)                                  |
| Global tags      | `/tags`                                    | Quản lý tag toàn hệ thống                                      |

**Screen Sets** (gom screens thành nhóm)

| Feature          | Route                                          | Purpose                      |
| ---------------- | ---------------------------------------------- | ---------------------------- |
| Screen Sets list | `/files/{fileKey}/screen-sets`                 | Danh sách screen sets        |
| Create / Edit    | `…/screen-sets/create` · `…/{frameSetId}/edit` | Tạo / chỉnh sửa screen set   |
| Set detail       | `…/{frameSetId}/list`                          | Xem screens trong 1 set      |
| Screen in set    | `…/{frameSetId}/screens/{screenId}`            | Mở screen từ context của set |

**Settings cấp độ file** (`/files/{fileKey}/settings`)

| Feature                | Route                             | Purpose                                  |
| ---------------------- | --------------------------------- | ---------------------------------------- |
| Project Overview       | `…/settings/overview`             | Mô tả project (markdown)                 |
| GitHub                 | `…/settings/github`               | OAuth + link repo (cho VSCode Ext / CLI) |
| Google                 | `…/settings/google`               | OAuth Google (cho Syncer Google Sheets)  |
| Screen Translation     | `…/settings/screen-translation`   | Hub đa ngôn ngữ                          |
| Translation Languages  | `…/screen-translation/languages`  | Bật/tắt ngôn ngữ cho file                |
| Translation Dictionary | `…/screen-translation/dictionary` | Bảng dịch en↔jp↔vi                     |
...
**Phân công ai cập nhật field nào** (workflow khuyến nghị):

| Field           | Người phụ trách | Khi nào chuyển `Done`                             |
| --------------- | --------------- | ------------------------------------------------- |
| `design_status` | Designer        | Hoàn tất thiết kế Figma cho screen                |
| `spec_status`   | BrSE / PM       | Spec của tất cả item trong screen đã được approve |
| `dev_status`    | Developer       | Hoàn tất implement feature liên quan đến screen   |
| `review_status` | QA / Reviewer   | Review / test hoàn tất, không còn blocking issue  |
...
## 4. MoMorph Syncer (Google Add-on - chỉ dành cho nhân viên Sun\*)
...
- Tài khoản **Sun\* Asterisk Google Workspace** (`@sun-asterisk.com`). Tài khoản Google cá nhân **không** truy cập được add-on.
...
- Nếu menu MoMorph Syncer không xuất hiện trong **Extensions**, liên hệ Slack `#con_momorph-support_all`.
...
Host tại `https://mcp.momorph.ai/mcp`, cung cấp **31 tools** cho AI agent đọc/ghi dữ liệu MoMorph & Figma. Người dùng **không gọi trực tiếp** — AI agent tự dùng (qua slash commands ở §7.2 hoặc qua prompt tự nhiên trong Claude Desktop §7.5). Mô tả chi tiết từng tool được trình bày ở tài liệu riêng **MoMorph MCP Server — Tools Reference (English)** (file `momorph-mcp-tools-reference-en.pdf`).
...
Để xem mô tả chi tiết, tham số, kiểu kết quả trả về và ví dụ sử dụng cho toàn bộ **31 tools** của MCP server, tham khảo tài liệu riêng được phát hành cùng bộ tài liệu này:

📘 **MoMorph MCP Server — Tools Reference (English)** — file `momorph-mcp-tools-reference-en.pdf` (tìm trong Slack channel `#con_momorph-support_all`).
...
VSCode Extension được phân phối qua file `.vsix`. Liên hệ Slack `#con_momorph-support_all` để nhận file mới nhất.
...
- **MCP servers** cấu hình sẵn (key trong `momorph.mcpServers`): `morpheus`, `context7`, `testViewpoints`
...
1. Liên hệ Slack `#con_momorph-support_all` để nhận file `momorph-mcp.mcpb` mới nhất.
...
**Menu MoMorph Syncer không xuất hiện trong Extensions** — add-on chưa được team vận hành deploy cho file Sheets này. Liên hệ Slack `#con_momorph-support_all`.

**"Missing authentication token" khi gọi action** — Google session hết hạn. Re-login Google account (đảm bảo dùng email `@sun-asterisk.com`), refresh tab Sheets, thử lại.
...
_Vấn đề khác? Slack `#con_momorph-support_all`._
